...
def profile(motifs,t):
    ...
    for motif in motifs:
        ...
        for nuc in motif:
            i=i+1
            profile[nuc][i]+=1
    profile_pseudo={'A':[ct+1 for ct in profile['A']],'C':[ct+1 for ct in profile['C']],'G':[ct+1 for ct in profile['G']],'T':[ct+1 for ct in profile['T']]}
    profile_prob={'A':[ct/(t+4) for ct in profile_pseudo['A']],'C':[ct/(t+4) for ct in profile_pseudo['C']],'G':[ct/(t+4) for ct in profile_pseudo['G']],'T':[ct/(t+4) for ct in profile_pseudo['T']]}
    profile_prob2={'A':[ct/t for ct in profile['A']],'T':[ct/t for ct in profile['T']],'C':[ct/t for ct in profile['C']],'G':[ct/t for ct in profile['G']]}
    return (profile_prob,profile,profile_prob2)#No need to return profile_pseudo as profile_prob is based on that
